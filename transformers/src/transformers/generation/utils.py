...
class GenerationMixin:
    ...
    def generate(
        ...
        rollout_length: Optional[int]=0,
        branching_factor: Optional[int]=0,
        roll_out_scorer = None,
        **kwargs,
        ...
        if is_greedy_gen_mode:
            ...
            # 13. run beam search with rollout
            return self.beam_search_with_rollout(
                input_ids,
                roll_out_scorer=roll_out_scorer,
    ...
    def beam_search_with_rollout(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            ########## Rollout for each top next token ##########
            
            # sort next tokens
            next_tokens_indexes = torch.argsort(next_token_scores_processed, dim=1, descending=True)
           
            rollout_scores_list = []
     
            # prepare kwargs
            new_model_kwargs = self._update_model_kwargs_for_generation(
                    outputs, model_kwargs.copy(), is_encoder_decoder=self.config.is_encoder_decoder
                )

            new_generation_config = copy.copy(self.generation_config)
            new_generation_config.top_p = 0.9
            logits_warper = self._get_logits_warper(new_generation_config)
 

            # rollout for each topn next tokens
            for i in range(branching_factor):
                new_input_ids = torch.cat([input_ids,  next_tokens_indexes[:, i].reshape(-1,1)], dim=-1)
                new_max_length =  rollout_length + new_input_ids.shape[1]
                # 12. expand input_ids with `num_return_sequences` additional sequences per batch
                new_input_ids, new_model_kwargs_ = self._expand_inputs_for_generation(
                    input_ids=new_input_ids,
                    expand_size=new_generation_config.num_return_sequences,
                    is_encoder_decoder=self.config.is_encoder_decoder,
                    **new_model_kwargs.copy(),
                )

                # Do rollouts
                rollout_output =  self.sample(
                    new_input_ids,
                    logits_processor=logits_processor,
                    logits_warper=logits_warper,
                    stopping_criteria=StoppingCriteriaList([MaxLengthCriteria(max_length=new_max_length)]), # max_length
                    pad_token_id=pad_token_id,
                    eos_token_id=eos_token_id,
                    output_attentions=output_attentions,
                ...
                # score each rollout
                rollout_scores_branch = roll_out_scorer.get_scores(rollout_output,original_input_ids)
                rollout_scores_list.append(rollout_scores_branch.reshape(batch_size,num_beams))
                
            rollout_scores = torch.cat(rollout_scores_list,dim=-1)
            rollout_scores_indexes = torch.argsort(rollout_scores, dim=1, descending=True)

            prune_branches =  rollout_scores_indexes[:,num_beams:].reshape(batch_size*num_beams,-1)
            ## update scores
            for j in range(input_ids.shape[0]):
                next_token_scores_processed[j,next_tokens_indexes[j,branching_factor:]] = -1e9
                next_token_scores_processed[j,next_tokens_indexes[j,prune_branches[j,:]]] = -1e9
            ##########
